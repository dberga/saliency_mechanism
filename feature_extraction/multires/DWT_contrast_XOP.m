function [w c] = DWT_contrast(image, wlev)
...
for s = 1:wlev
    ...
    HHF = upsample(upsample(HHF,2)',2)';A=[ones(64,32),zeros(64,16),2.*ones(64,32),zeros(64,16),ones(64,32)];
    ...
    DF = orig_image - (image + HGF + GHF);
	 w{s,1}(:,:,3) = DF;
	 ind=find(abs(orig_image+image)>1.e-4);
	 
	 
	 kk=w{s,1}(:,:,3);
	 w{s,1}(:,:,3)=0.;
	 kk(ind)=kk(ind)./ (orig_image(ind)+image(ind));
	 w{s,1}(:,:,3) = kk;
	 kk=w{s,1}(:,:,2);
	 w{s,1}(:,:,2)=0.;
	 kk(ind)=kk(ind)./ (orig_image(ind)+image(ind));
